...
def check_install_unambiguity(readme_path: str) -> list[tuple[str, str]]:
    ...
    if js_found and not re.search(r"node[.]js\s*[><=]+\s*[\d.]+|node\s+--version", combined):
        messages.append(("ERROR", "JavaScript package manager found in ## Installation without Node.js version constraint. Add a line like: Requires: Node.js >= 18.0.0"))

    pip_found = re.findall(r"\bpip3?\s+install", block_text)
    if pip_found and not re.search(r"pip install\s+['\"]?[a-zA-Z][\w-]+[=><][\d.]+", block_text):
        messages.append(("WARNING", "pip install found without explicit version constraint. Add e.g. pip install package==1.0.0"))
    ...
    if not has_verification and install_lines:
        messages.append(("ERROR", "No verification command found in ## Installation block. Add a step like: `program --version` or `# Verify: should output X`"))
    ...
    if pm_count >= 2:
        ...
        if not any(re.search(p, block_text, re.IGNORECASE) for p in disambig):
            messages.append(("WARNING", f"{pm_count} package managers found without disambiguation. Add (recommended), (macOS), or (choose one) to each."))
