...
async function main() {
  return await withElectronPage(async (page) => {
    ...
    // Click the event title directly (not the harness)
    ...
    if (allD.indexOf('drawer:') >= 0) {
      if (allD.indexOf('01:30') >= 0) {
        console.log('=== OLD EVENT (01:30 wrong time) ===');
      } else {
        console.log('\n=== NEW EVENT DETAIL ===');
      }
      console.log(allD);

      // Check if there's an edit/delete button
      const hasEdit = allD.indexOf('编辑') >= 0;
      const hasDel = allD.indexOf('删除') >= 0;
      console.log('Has edit:', hasEdit, 'Has delete:', hasDel);
    } else {
      console.log('No drawer opened — trying to find and click the event span directly');
      // Fallback: try clicking the fc-event main element
