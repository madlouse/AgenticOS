...
- **设计（统一 Entry 模型，two-tier 承载;2026-06-12 评审修订）**：把上述收敛为一条带类型的进化日志条目。承载体分两层：
  - `distillation-ledger` 维持现职——机器本地 capture 队列 + 蒸馏状态机（可含 private refs），仅做加固（锁/原子写/损坏备份/passthrough）;
  - **蒸馏后的进化条目落 git-tracked 的 `<context>/evolution-log/YYYY-MM.yaml`**——append-only、按月分片（压缩并行 worktree 合并冲突面）、record full-mode/drain 时自动写、`refs.issue` 从 worktree 分支名自动盖戳（确定性收口，不靠 agent 自觉）、随 `agenticos_save` 提交、人可直接读、跨机器随 repo 走。
  - 原"扩展 ledger 为承载体"方案经三路评审否决：ledger 是 gitignore 的单机 sidecar，把人机共享时间线放它上面，跨机器为空、人读不到，且把 git-tracked 的 decisions 降级为单机文件投影。

  ```yaml
  # <context>/evolution-log/2026-06.yaml
  - id: evo-2026-06-12-<hash>                          # kind+summary+issue 哈希,幂等去重
    at: 2026-06-12T...
    kind: decision | case | knowledge_ref              # 类型化
    summary: "M2 精度采样改用分层抽样而非随机"
    rationale: "随机抽样在低频类别上方差过大（#355 评测证据）"   # 为什么(可选,agent 判断)
    refs:                                              # 可回溯
      issue: "#355"                                    # 自动盖戳
      pr: "#xxx"
      knowledge: [knowledge/m2-precision-sampling.md]
  ```
  `state.working_memory.decisions` 与 `case` **照旧直写**（evolution log 为纯增量层）;"变投影"（580b）推迟到时间线证明自身价值后再评估——YAGNI。
...
- **设计（分级召回，克制）**：新增 `agenticos_recall({ query?, issue_id?, task? }) → top-N entries+docs`。**v1 不上向量库**（避免过度设计），用确定性信号:
  ...
  - **接入冷启动（2026-06-12 评审修订）**：召回在 `issue_bootstrap` **内部服务端自动生成**,写入独立的 `recalled` 输出字段（不复用 `additional_context`——评审实证该字段是 caller→evidence 的登记输入,不是 system→agent 的注入通道）,并经 `agenticos_issue_start` 的 READY payload 同级透出。验收判据是注意力流而非数据流：零先验 agent 走标准入口后,上下文中可见召回内容。
...
### P1 — 召回最小闭环（补最大的洞）
- **L2** Evolution Log 统一模型(2026-06-12 修订):ledger 加固 + git-tracked `evolution-log/YYYY-MM.yaml` 类型化时间线(record/drain 自动写,refs.issue 分支自动盖戳);decisions/case 照旧直写,投影推迟(YAGNI)。
...
- 不新增真相源(2026-06-12 修订:原"不新建第五套存储"克制错了对象——真实约束是共享可见性,不是存储数量)——Evolution Log 是 decisions/case 碎片的收敛目的地,不是又一套并行真相;ledger 维持 capture 队列职责不变。
