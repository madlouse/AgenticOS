...
export const AGENTICOS_SKILL_TEMPLATE_VERSION = 6;
...
4. After \`agenticos_switch\` succeeds, immediately apply the returned \`project_workdir\` / \`explicit_workdir\` / project path as the active filesystem workdir. Codex-style runtimes must pass that path as explicit tool \`workdir\`; worker launchers should start new Codex sessions with \`codex -C <project_path>\`; Claude Code must use per-call cwd guidance or absolute paths; Hermes Agent should rely on its installed AgenticOS cwd applicator when present.
5. After \`agenticos_switch_out\` succeeds, immediately apply its returned \`target_workdir\` / \`explicit_workdir\` as the active filesystem workdir. Do not keep using the previous project path after switch-out. MCP cannot mutate the parent process cwd by itself; if the current agent cannot apply \`target_workdir\`, say the switch-out effect is not complete before continuing.
